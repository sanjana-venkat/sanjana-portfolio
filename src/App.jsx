...
function HeroTile() {
  return (
    <>
      {/* DESKTOP hero — untouched from doc5 */}
      <div
      ...
      >
        ...
        <div className="absolute overflow-hidden" style={{ width: "200px", height: "240px", bottom: "-55px", right: "32px", borderRadius: "22px", border: "8px solid #FFFFFF", boxShadow: "0px 2.8px 4.2px -0.7px rgba(0,0,0,0.1), 0px 1.4px 2.8px -1.4px rgba(0,0,0,0.1)", transform: "rotate(9.83deg)", zIndex: 30 }}>
          <img src="/profile.jpg" alt="Sanjana Venkat" className="w-full h-full object-cover object-center" />
        </div>
      </div>

      {/* MOBILE hero — untouched from doc5 */}
      <div
        className="relative overflow-hidden rounded-[32px] p-8 pb-0 flex flex-col lg:hidden"
        style={{ background: "rgba(156, 63, 20, 0.17)", minHeight: "420px" }}
      >
        <h1 className={`font-semibold leading-[1.2] tracking-[-0.02em] text-[#9C3F14] ${HEADING}`} style={{ fontSize: "36px" }}>
          Sanjana Venkat
        </h1>
        <p className="mt-2 leading-[1.5]" style={{ color: "#57423A", fontSize: "15px" }}>
        ...
        <div className="flex justify-center mt-8 flex-1 items-end">
          <div className="overflow-hidden" style={{ width: "100%", height: "280px", borderRadius: "22px 22px 0 0" }}>
  ...
  return (
    <article className={`rounded-[32px] bg-white p-6 text-[14px] h-full flex flex-col justify-between ${BODY}`} style={{ minHeight: "220px" }}>
      <div>
        ...
        {/* MOBILE FIX 1: "I study psychology" */}
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <a href="/SanjanaVenkat_ProductDesign_Resume.pdf" target="_blank" rel="noreferrer" className={`inline-flex items-center gap-1 text-[13px] text-[#8A817B] underline underline-offset-4 hover:text-[#A5522A] transition ${HEADING}`}>
          resume →
        </a>
      </div>
    </article>
...
// MOBILE FIX 2: fixed image height constant
const MOBILE_IMG_HEIGHT = 260;
...
function NavTile() {
  ...
  const linePath    = item.lineStyle === "very-wavy" ? LINE_VERY_WAVY : item.lineStyle === "slightly-wavy" ? LINE_SLIGHTLY_WAVY : LINE_STRAIGHT;
  ...
  return (
    ...
    >
        ...
        <div className="absolute inset-0 flex flex-col px-6 pt-6 pb-5">
          ...
          <div className="flex-1 relative overflow-hidden">
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 320 120" preserveAspectRatio="xMidYMid meet">
              <path d={SCRIBBLE_PATH} fill="none" stroke="#D4CBC6" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
      ...
      {active && (
        <>
          <p className={`pt-6 px-6 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#9A8176] shrink-0 ${HEADING}`}>my story</p>

          <div className="relative flex-1 flex flex-col justify-between overflow-hidden">

            {/* DESKTOP: image + line centered, content at bottom — identical to doc5 */}
            <div className="flex-1 relative lg:block hidden">
              {item.img && (
                <div key={`img-${step}`} className="absolute overflow-hidden"
                  style={{ top: "50%", left: "-8px", transform: imgVisible ? "translateY(-50%) rotate(-5deg)" : "translateY(-50%) rotate(-5deg) translateX(-10px)", width: "110px", height: "150px", borderRadius: "20px", border: "5px solid #FFFFFF", boxShadow: "0 4px 16px rgba(0,0,0,0.13)", opacity: imgVisible ? 1 : 0, transition: "opacity 0.35s ease, transform 0.45s cubic-bezier(0.22,1,0.36,1)", zIndex: 10 }}>
                  <img src={item.img} alt={item.label} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} />
                </div>
              )}
              <svg className="absolute left-0 right-0 w-full h-[78px]" viewBox="0 0 320 80" preserveAspectRatio="none" style={{ top: "50%", transform: "translateY(-50%)", overflow: "visible" }}>
                {phase === "scribble" && (
                  <path key="scribble" d={SCRIBBLE_PATH} fill="none" stroke="#2F2F2F" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"
                    style={{ strokeDasharray: SCRIBBLE_LEN, strokeDashoffset: SCRIBBLE_LEN, animation: "tlScribbleDraw 0.6s cubic-bezier(0.4,0,0.6,1) forwards" }} />
                )}
                {showLine && phase !== "scribble" && (
                  <path key={`line-${step}-${phase}`} d={linePath} fill="none" stroke={strokeColor} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"
                    style={{ strokeDasharray: 520, strokeDashoffset: phase === "line" ? 520 : 0, animation: phase === "line" ? "tlDraw 0.45s cubic-bezier(0.22,1,0.36,1) forwards" : "none" }} />
                )}
                {item.heart && showContent && (
                  <g key={`heart-${step}`}>
                    <path d="M 160 43 C 153 38 149 33 149 30 C 149 26 152 24 155 25 C 157 26 159 28 160 30 C 161 28 163 26 165 25 C 168 24 171 26 171 30 C 171 33 167 38 160 43 Z"
                      fill="none" stroke={strokeColor} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"
                      style={{ strokeDasharray: 80, strokeDashoffset: 80, animation: "tlHeartDraw 0.55s cubic-bezier(0.4,0,0.2,1) forwards" }} />
                  </g>
                )}
                {showContent && !item.heart && (
                  <circle key={`dot-${step}`} cx="160" cy="38" r="5"
                    fill={item.isNow ? "#D96F45" : "white"} stroke={item.isNow ? "#D96F45" : strokeColor} strokeWidth="1.6"
                    style={{ animation: phase === "content" ? item.isNow ? "timelineDotSlide 0.38s cubic-bezier(0.22,1,0.36,1) both, tlNowGlow 1.8s ease-in-out 0.5s infinite" : "timelineDotSlide 0.38s cubic-bezier(0.22,1,0.36,1) both" : item.isNow ? "tlNowGlow 1.8s ease-in-out 0.5s infinite" : "none", filter: item.isNow ? "drop-shadow(0 0 5px rgba(217,111,69,0.8))" : "none" }} />
                )}
              </svg>
            </div>

            {/* ── MOBILE LAYOUT (FIX 3) ──
                Order: SVG line → text content (reserved height) → image (fixed, flush to bottom)
                No height jumps: content has minHeight so image never shifts position.
            */}
            <div className="lg:hidden flex flex-col">
              {/* 1. SVG line */}
              <div style={{ height: "60px", position: "relative", flexShrink: 0 }}>
                <svg className="absolute left-0 right-0 w-full h-full" viewBox="0 0 320 60" preserveAspectRatio="none" style={{ overflow: "visible" }}>
                  {phase === "scribble" && (
                    <path d={SCRIBBLE_PATH} fill="none" stroke="#2F2F2F" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"
                      style={{ strokeDasharray: SCRIBBLE_LEN, strokeDashoffset: SCRIBBLE_LEN, animation: "tlScribbleDraw 0.6s cubic-bezier(0.4,0,0.6,1) forwards" }} />
                  )}
                  {showLine && phase !== "scribble" && (
                    <path key={`line-m-${step}-${phase}`}
                      d={item.lineStyle === "very-wavy" ? MOBILE_VERY_WAVY : item.lineStyle === "slightly-wavy" ? MOBILE_SLIGHTLY_WAVY : MOBILE_STRAIGHT}
                      fill="none" stroke={strokeColor} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"
                      style={{ strokeDasharray: 520, strokeDashoffset: phase === "line" ? 520 : 0, animation: phase === "line" ? "tlDraw 0.45s cubic-bezier(0.22,1,0.36,1) forwards" : "none" }} />
                  )}
                  {item.heart && showContent && (
                    <g key={`heart-m-${step}`}>
                      <path d="M 160 36 C 153 31 149 26 149 23 C 149 19 152 17 155 18 C 157 19 159 21 160 23 C 161 21 163 19 165 18 C 168 17 171 19 171 23 C 171 26 167 31 160 36 Z"
                        fill="none" stroke={strokeColor} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"
                        style={{ strokeDasharray: 80, strokeDashoffset: 80, animation: "tlHeartDraw 0.55s cubic-bezier(0.4,0,0.2,1) forwards" }} />
                    </g>
                  )}
                  {showContent && !item.heart && (
                    <circle key={`dot-m-${step}`} cx="160" cy="30" r="5"
                      fill={item.isNow ? "#D96F45" : "white"} stroke={item.isNow ? "#D96F45" : strokeColor} strokeWidth="1.6"
                      style={{ animation: phase === "content" ? item.isNow ? "timelineDotSlide 0.38s cubic-bezier(0.22,1,0.36,1) both, tlNowGlow 1.8s ease-in-out 0.5s infinite" : "timelineDotSlide 0.38s cubic-bezier(0.22,1,0.36,1) both" : item.isNow ? "tlNowGlow 1.8s ease-in-out 0.5s infinite" : "none", filter: item.isNow ? "drop-shadow(0 0 5px rgba(217,111,69,0.8))" : "none" }} />
                  )}
                </svg>
              ...
              {/* 2. Text content — fixed reserved height so image never shifts */}
              <div key={`mob-content-${step}`} className="px-6 text-center"
                style={{ opacity: showContent ? 1 : 0, minHeight: "110px", paddingTop: "4px", paddingBottom: "20px", animation: phase === "content" ? "timelineContentSlide 0.42s cubic-bezier(0.22,1,0.36,1) both" : "none" }}>
                <div className={`text-[26px] font-bold ${item.isNow ? "text-[#D96F45]" : "text-[#1A1A1A]"} ${HEADING}`}>{item.year}</div>
                <div className={`text-[13px] font-semibold text-[#9A8176] mt-0.5 ${HEADING}`}>{item.label}</div>
                <p className="mt-1 text-[12px] leading-[1.45] text-[#5F5149]">{item.sub}</p>
              </div>

              {/* 3. Image — fixed height, flush to tile edges, rounded top corners only */}
              <div key={`img-mob-${step}`}
                style={{ height: `${MOBILE_IMG_HEIGHT}px`, flexShrink: 0, overflow: "hidden", borderRadius: "20px 20px 0 0", opacity: imgVisible ? 1 : 0, transform: imgVisible ? "translateY(0px)" : "translateY(12px)", transition: "opacity 0.35s ease, transform 0.45s cubic-bezier(0.22,1,0.36,1)" }}>
                <img src={item.img} alt={item.label} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top" }} onError={(e) => { e.currentTarget.style.display = "none"; }} />
              </div>
            </div>

            {/* Desktop content label — identical to doc5 */}
            <div
              key={`content-${step}`}
              className="px-6 pb-4 text-left lg:text-left text-center shrink-0"
              style={{ opacity: showContent ? 1 : 0, animation: phase === "content" ? "timelineContentSlide 0.42s cubic-bezier(0.22,1,0.36,1) both" : "none" }}
            >
              ...
              <p className="mt-1 text-[12px] leading-[1.45] text-[#5F5149] whitespace-nowrap lg:text-left text-center">{item.sub}</p>
            </div>
          </div>
        </>
      )}
      ...
      <style>{`
        @keyframes tlScribbleDraw { from { stroke-dashoffset: ${SCRIBBLE_LEN}; } to { stroke-dashoffset: 0; } }
        ...
        @keyframes tlHeartDraw { from { stroke-dashoffset: 80; } to { stroke-dashoffset: 0; } }
        @keyframes tlNowGlow { 0%,100% { filter: drop-shadow(0 0 3px rgba(217,111,69,0.5)); } 50% { filter: drop-shadow(0 0 8px rgba(217,111,69,0.9)); } }
      `}</style>
    </div>
  );
}
    ...
    <div className={`group relative w-full rounded-[32px] bg-white flex flex-col text-left overflow-hidden ${BODY}`}>
      ...
      <div style={{ display: "flex", gap: "12px", marginLeft: "-12px", marginRight: "-12px", marginBottom: "-2px" }}>
        {WORK_PREVIEWS.map((proj) => (
          <div key={proj.label} className="relative flex-1 overflow-hidden" onClick={() => onOpenProject(proj.projectKey)}
            style={{ height: "200px", borderRadius: "16px 16px 0 0", border: "5px solid rgba(221, 192, 182, 0.45)", borderBottom: "none", boxShadow: "0px 2.8px 4.2px -0.7px rgba(0,0,0,0.1), 0px 1.4px 2.8px -1.4px rgba(0,0,0,0.1)", background: "#EDEAE7", minWidth: 0, cursor: "pointer" }}>
            <img src={proj.src} alt={proj.label} className="w-full h-full object-cover object-top" onError={(e) => { e.target.style.opacity = "0"; }} />
            <div className="absolute inset-0 flex items-end transition-opacity duration-200 opacity-0 hover:opacity-100" style={{ background: "rgba(0,0,0,0.5)" }}>
...
function TestimonialTile() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => setIndex((c) => (c + 1) % TESTIMONIALS.length), 5200);
  ...
  return (
    <article className={`relative rounded-[32px] bg-white p-7 text-[13px] ${BODY}`} style={{ height: "100%", overflow: "hidden" }}>
      ...
      <div key={index} className="animate-[slideIn_0.35s_ease_forwards]" style={{ height: "130px", overflow: "hidden" }}>
        <p className="italic leading-[1.55] text-[#4F4741]" style={{ display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden" }}>"{quote}"</p>
      ...
      <button type="button" onClick={() => setIsPaused((c) => !c)} aria-label={isPaused ? "Play testimonials" : "Pause testimonials"}
        ...
        <span className="flex h-full w-full items-center justify-center leading-none">{isPaused ? <PlayIcon /> : <PauseIcon />}</span>
      </button>
    </article>
  );
}
...
function ResponseLinks({ active, openProjectForActivePill }) {
  if (active === "let's talk AI") {
    return (
      <div className="flex flex-wrap gap-x-6 gap-y-3 px-2 pt-4 animate-[fadeUp_0.35s_ease_forwards]">
        <button onClick={() => openProjectForActivePill("ai-framer")} className={`inline-flex text-[14px] font-medium text-[#8A817B] underline underline-offset-4 transition-colors hover:text-[#A5522A] ${HEADING}`}>AI chat journeys →</button>
        <button onClick={() => openProjectForActivePill("casey-ai")} className={`inline-flex text-[14px] font-medium text-[#8A817B] underline underline-offset-4 transition-colors hover:text-[#A5522A] ${HEADING}`}>what's casey AI? →</button>
      </div>
    );
  }
  const ctaText = (() => {
  ...
  return (
    <div className="px-2 pt-4 animate-[fadeUp_0.35s_ease_forwards]">
      <button onClick={() => openProjectForActivePill()} className={`inline-flex text-[14px] font-medium text-[#8A817B] underline underline-offset-4 transition-colors hover:text-[#A5522A] ${HEADING}`}>{ctaText}</button>
    </div>
  );
}
  ...
  return (
    <>
      <div className="mb-6 flex justify-end">
        <div className={`rounded-[48px_48px_0px_48px] bg-[#A5522A] px-6 py-3 text-[14px] leading-[1.8] text-white animate-[messageSend_0.35s_ease_forwards] ${TYPEWRITE}`}>{active}</div>
      </div>
      {showThinking && (
        <div className="rounded-[0px_36px_36px_36px] bg-white p-5 animate-[fadeUp_0.25s_ease_forwards] sm:p-6">
          <div className={`flex items-center gap-2 text-[12px] text-[#8A817B] ${HEADING}`}>
            <span className="h-2 w-2 rounded-full bg-[#A5522A] animate-pulse" />thinking
          </div>
        </div>
      )}
      {showResponse && (
  ...
  return (
    <div className={`fixed inset-0 z-[60] flex flex-col bg-[#FFF8F5] ${BODY}`}>
      <div className="pointer-events-none fixed left-0 right-0 top-0 z-10 flex items-start justify-between gap-3 px-4 pt-4">
        <div className="pointer-events-auto max-w-[calc(100%-72px)] rounded-[999px] border border-[#E4E2E1] bg-white px-7 py-5 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setShowHint((c) => !c)} aria-label={showHint ? "Hide hint" : "Show hint"} className="flex h-4 w-4 items-center justify-center p-0 leading-none text-[#8A817B] transition hover:text-[#A5522A]">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-32 pt-28">
        <ChatConversation active={active} showThinking={showThinking} showResponse={showResponse} showPills={showPills} showUserNeedsRest={showUserNeedsRest} onTypeDone={onTypeDone} openProjectForActivePill={openProjectForActivePill} instantType={instantType} />
      </div>
        ...
        <div className="pointer-events-auto no-scrollbar flex gap-2 overflow-x-auto">
          {PILLS.map((pill) => (
            <button key={pill} onClick={() => setActive(pill)}
              className={`shrink-0 rounded-full border bg-white px-4 py-2 text-[11px] transition ${active === pill ? "border-[#A5522A] text-[#A5522A]" : "border-[#E4E2E1] text-[#6B625C] hover:border-[#D8C5BB]"} ${HEADING}`}>
              {pill}
            </button>
          ))}
        </div>
...
export default function PortfolioHome() {
  ...
  useEffect(() => {
    ...
    const removeExistingFavicons = () => { document.querySelectorAll("link[rel*='icon'], link[rel='apple-touch-icon']").forEach((link) => { if (link.parentNode) link.parentNode.removeChild(link); }); };
    const addFavicon = (rel, href, type = "image/jpeg") => { const link = document.createElement("link"); link.rel = rel; link.type = type; link.href = `${href}?v=${Date.now()}`; document.head.appendChild(link); };
  ...
  useEffect(() => {
    ...
    if (hash.startsWith("#work=")) { const slug = hash.replace("#work=", ""); setWorkProjectSlug(slug || "b2c"); setProjectOpen("work-browser"); }
  }, []);

  useEffect(() => {
    if (isFirstLoad.current) {
      isFirstLoad.current = false;
      setInstantType(true); setShowThinking(false); setShowResponse(true); setShowPills(true);
      setShowUserNeedsRest(active === "how do you uncover user needs");
      return;
    }
    setInstantType(false); setShowPills(false); setShowResponse(false); setShowUserNeedsRest(false); setShowThinking(true);
    const timer = setTimeout(() => { setShowThinking(false); setShowResponse(true); }, 850);
    return () => clearTimeout(timer);
  }, [active]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleTypeDone = () => { if (active === "how do you uncover user needs") setShowUserNeedsRest(true); setShowPills(true); };
  const handlePillSelect = (pill) => { if (pill === active) return; chatCardRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }); setActive(pill); };
  const openWorkProject = (slug) => { setWorkProjectSlug(slug); window.history.replaceState(null, "", `#work=${slug}`); setProjectOpen("work-browser"); };

  const handleNav = (item) => {
    if (item === "my work") { setWorkProjectSlug("b2c"); window.history.replaceState(null, "", "#work=b2c"); setProjectOpen("work-browser"); return; }
    if (item === "what are you building now") { window.open(TRAVEL_DNA_URL, "_blank"); return; }
    if (item === "resume") { window.open(RESUME_URL, "_blank"); return; }
    if (item === "github") { window.open(GITHUB_URL, "_blank"); return; }
  ...
  const openProjectForActivePill = (override) => {
    ...
    if (active === "how do you uncover user needs") { openWorkProject("b2c"); return; }
    ...
    if (active === "how do you ship fast")          { window.open("https://travel-dna-kohl.vercel.app/", "_blank"); return; }
    if (active === "how do you get exec buy-in")    { openWorkProject("exec-pitch"); return; }
    if (active === "model design")                  { window.open("https://travel-dna-kohl.vercel.app/", "_blank"); }
  };
  ...
  return (
    ...
    >
      ...
      {projectOpen === "work-browser" && (
        <WorkBrowserModal initialSlug={workProjectSlug} onClose={() => { setProjectOpen(null); if (window.location.hash.startsWith("#work=")) window.history.replaceState(null, "", window.location.pathname); }} />
      )}
      ...
      {mobileChatOpen && <MobileChatModal active={active} setActive={setActive} showThinking={showThinking} showResponse={showResponse} showPills={showPills} showUserNeedsRest={showUserNeedsRest} onTypeDone={handleTypeDone} openProjectForActivePill={openProjectForActivePill} onClose={() => setMobileChatOpen(false)} instantType={instantType} />}
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1280px]">
        ...
        {/* ── DESKTOP — identical to doc5, zero changes ── */}
        <div
        ...
        >
          {/* Chat: col 1, rows 1-2, height 560px — doc5 original */}
          <div
            ref={chatCardRef}
          ...
          >
            ...
            <div ref={(el) => { chatScrollRef.current = el; _chatScrollEl = el; }} data-chat-scroll className="flex-1 overflow-y-auto px-6 no-scrollbar" style={{ paddingBottom: showPills ? "64px" : "16px" }}>
              <ChatConversation active={active} showThinking={showThinking} showResponse={showResponse} showPills={showPills} showUserNeedsRest={showUserNeedsRest} onTypeDone={handleTypeDone} openProjectForActivePill={openProjectForActivePill} instantType={instantType} />
            </div>
            {showPills && (
                  ...
                  <div className="flex gap-2" style={{ width: "max-content" }}>
                    {PILLS.map((pill) => (
                      <button key={pill} onClick={() => handlePillSelect(pill)}
                        className={`rounded-full border px-4 py-2 text-[11px] whitespace-nowrap transition hover:scale-[1.02] backdrop-blur-sm ${active === pill ? "bg-white border-[#A5522A] text-[#A5522A]" : "bg-white border-[#E4E2E1] text-[#6B625C] hover:border-[#D8C5BB]"} ${HEADING}`}>
                        {pill}
                      </button>
                    ))}
                  </div>
          ...
          <div style={{ gridColumn: "1 / 3", gridRow: "3" }}>
            <MyWorkTile onOpenProject={(key) => { const slugByKey = { "marketing-tiles": "ai-personalization", "ai-framer": "ai-chat-journeys", "travel-dna": "model-design" }; openWorkProject(slugByKey[key] || "b2c"); }} />
          </div>
        ...
        {/* ── MOBILE stack — MOBILE FIX 4: NavTile bare, no wrapper ── */}
